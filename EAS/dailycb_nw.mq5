...
input bool use_trailing_sl = true; // Use trailing stop loss (previous day low/high)
...
datetime lastTrailingSLUpdate = 0;
double bal;
double ini_bal;
...
int OnInit()
  {
   ...
   message = StringFormat("DailyCB Scalper EA Initialized: User: %s, Symbol: %s, Account Balance: %s at %s",
                          user,_Symbol,IntegerToString(ini_bal), current_time);
...
void OnDeinit(const int reason)
  {
   current_time = TimeToString(TimeCurrent(), TIME_DATE | TIME_MINUTES);
   message = StringFormat("DailyCB Scalper EA Deinitialized: User: %s, Symbol: %s, Account Balance: %s at %s",
                          user,_Symbol,IntegerToString(ini_bal), current_time);
   SendTelegramMessage(message);
   if(atrHandle != INVALID_HANDLE)
      ...
      if(m_trade.Buy(LotSize, _Symbol, curr_ask_price, stopLoss, 0, m_comment))
        {
         ...
         message = StringFormat("DailyCB Scalper EA (%s): BUY position opened: Symbol: %s Price: %s at %s",
                                user,_Symbol,DoubleToString(curr_ask_price), current_time);
         SendTelegramMessage(message);
        }
      else
      ...
      if(m_trade.Sell(LotSize, _Symbol, curr_bid_price, stopLoss, 0, m_comment))
        {
         ...
         message = StringFormat("DailyCB Scalper EA (%s): SELL position opened: Symbol: %s Price: %s at %s",
                                user,_Symbol,DoubleToString(curr_bid_price), current_time);
         SendTelegramMessage(message);
        }
      else
...
// Trailing stop loss function using previous day's low/high
void trailing_stop_loss()
  {
   ...
   datetime currentDayTime = iTime(_Symbol, PERIOD_D1, 0);
   if(lastTrailingSLUpdate == currentDayTime)
   ...
   if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
     {
      // For BUY: use previous day's LOW as trailing SL
      double prevDayLow = iLow(_Symbol, PERIOD_D1, 1);
      newSL = NormalizeDouble(prevDayLow, _Digits);
      ...
      if(newSL > currentSL)
        {
         if(m_trade.PositionModify(_Symbol, newSL, 0))
           {
            lastTrailingSLUpdate = currentDayTime;
            Print("✅ BUY Trailing SL updated: ", currentSL, " → ", newSL, " (Prev Day Low)");
           }
         else
     ...
     {
      // For SELL: use previous day's HIGH as trailing SL
      double prevDayHigh = iHigh(_Symbol, PERIOD_D1, 1);
      newSL = NormalizeDouble(prevDayHigh, _Digits);
      ...
      if(newSL < currentSL)
        {
         if(m_trade.PositionModify(_Symbol, newSL, 0))
           {
            lastTrailingSLUpdate = currentDayTime;
            Print("✅ SELL Trailing SL updated: ", currentSL, " → ", newSL, " (Prev Day High)");
           }
         else
